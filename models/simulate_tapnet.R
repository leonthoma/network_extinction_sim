...
#'
#' @export
simulate_tapnet_aug <- function(nlower, # Species number in lower trophic level
                            ...
                            names = sp_names, # List of species names
                            ctrb_vec = c(NULL),
                            initial_sim = FALSE
) {
  ...
  tree_low$tip.label <- sample(names$plants, nlower)
  tree_high <- pbtree(n = nhigher, scale = 1, nsim = 1)
  tree_high$tip.label <- sample(names$animals, nhigher)
  ...
  for (i in 1:Nwebs) {
    ...
    rew_probs_temp <- simnetfromtap_aug(webs[[i]]$traits, webs[[i]]$abuns, paramsList, webs[[i]]$pems,
                                        tmatch_type_pem, tmatch_type_obs, ctrb_vec, initial_sim = initial_sim)
    rew_probs[[i]]$A_mat <- rew_probs_temp$A_mat
    rew_probs[[i]]$T_mat <- rew_probs_temp$T_mat
    rew_probs[[i]]$L_mat <- rew_probs_temp$L_mat
    webs[[i]]$I_mat <- rew_probs_temp$I_mat
  ...
  sim_tapnet <- list(trees = list(low = tree_low, high = tree_high),
                     ...
                     tmatch_type_obs = tmatch_type_obs, rew_probs = rew_probs)
